...
parser = argparse.ArgumentParser(description="Stress Prediction Retrieval")
...
if not args.guideq.endswith('.csv'):
    raise ValueError("GuideQ dataset path must end with .csv")
...
def gtr_retrieval(k=3):
    ...
    model = SentenceTransformer('sentence-transformers/gtr-t5-base',
                              cache_folder="/nfs/kundeshwar/ashutosh/.cache").half()
    index = faiss.read_index('stress_index.faiss')
    ...
    output_path = f"results/stress_top_{k}_gtr.csv"
